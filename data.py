...
def translate(episode, gamma=0.9, penalty=-1.0):
    ...
    # Task 0: separate update record from rec_valid
    i_move = [i for i, rec in enumerate(rec_valid) if rec[0] == "move"]
    # scores ["civilian", "conflict", "science", "commerce", "guild", "wonder", "wealth", "coin", "total"]
    for i in i_move:
        # # rec: (api, args, res, reward)
        if rec_valid[i - 1][0] == "update":
            _, scores, _, _ = rec_valid[i - 1] # scores (9, n_player)
            scores = [score[0] for score in scores] # only predict the score for itself to simplify scaling problem
        else:
            scores = [0] * 9
        api, args, res, reward = rec_valid[i]
        rec_valid[i] = [api, args, res, scores, reward] # rec: (api, args, res, scores, reward)
    rec_valid = [rec for rec in rec_valid if rec[0] != "update"]
    ...
    #rec_valid = [*rec for rec, rec_ in zip(rec_valid, rec_update)]
    # Task 3 : add penalty to rec_invalid & sample n examples from rec_invalid
    if rec_invalid:
        rec_invalid_ = [[rec[0], rec[1], rec[2], [0] * 9, penalty] for rec in rec_invalid]
    ...
    ss = defaultdict(list)
    for rec in recs:
        api, (state, record, hand), (pick, action), s, r = rec
    ...
    for key in vs:
        ...
        for item in [rs, ss]:
            item[key] = np.array(item[key], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    p_model = "model_float16/ac.keras"
    p_other = "model_float16/base.keras"
    ...
    # parse TFRecord
    dataset = tf.data.TFRecordDataset([p_data])
    dataset = dataset.map(parse_example)
    for v, h, y, r, s in dataset:
        print(v.shape, h.shape, y.shape, r.shape, s.shape)
        print(v.dtype, h.dtype, y.dtype, r.dtype, s.dtype)
